...
export default function PlayerList(props: { className?: string, lobbyId: string, playerId: string, hideKick?: boolean }) {
    ...
    <div className={props.className}>
      <ul>
        {lobby?.players.map((player) => {
          return (
            <li key={player.id} className={`flex justify-around items-center p-2 my-2 w-full ${player.id === props.playerId ? 'bg-stone-700' : ''}`}>
